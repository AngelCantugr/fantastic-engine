...
export class OAuthClient {
  ...
  async getAccessToken(): Promise<string> {
    ...
    if (this.accessToken && this.tokenExpiry && Date.now() < this.tokenExpiry) {
      console.log('[MCP-OAuth] Using cached access token');
      return this.accessToken;
    }

    // Fetch new token
    console.log('[MCP-OAuth] Fetching new access token...');
  ...
  private async fetchAccessToken(): Promise<void> {
    try {
      ...
      console.log('[MCP-OAuth] ✓ Access token obtained successfully');
      console.log(`[MCP-OAuth]   Scopes: ${response.data.scope}`);
      console.log(`[MCP-OAuth]   Expires in: ${response.data.expires_in}s`);
    } catch (error) {
      if (axios.isAxiosError(error)) {
        console.error('[MCP-OAuth] ✗ Failed to fetch token:', error.response?.data);
    ...
    try {
      ...
      if (axios.isAxiosError(error)) {
        if (error.response?.status === 401) {
          ...
          console.log('[MCP-OAuth] Token invalid, clearing cache and retrying...');
